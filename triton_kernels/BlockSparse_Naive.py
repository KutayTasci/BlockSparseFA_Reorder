...
@triton.jit
def _attn_fwd_block_sparse(sm_scale, M,  #
              Z, H, desc_q, desc_k, desc_v, desc_o, block_mask, N_CTX,  #
    ...
    q_ptrs = desc_q + (off_m[:, None] * HEAD_DIM) + off_k[None, :]
    q = tl.load(q_ptrs)
    # stage 1: off-band
    ...
    o_ptrs = desc_o + (off_m[:, None] * HEAD_DIM) + off_k[None, :]
